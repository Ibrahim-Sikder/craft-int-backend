...
loanSchema.pre('save', function(next) {
  ...
  if (this.repayment_end_date) {
    ...
    if (this.remainingBalance <= 0) {
      ...
      this.status = 'overdue';
    } else if (this.status === 'overdue' && new Date() <= this.repayment_end_date) {
      this.status = 'active';
    }
  }
  
  // Auto-calculate monthly installment if not provided
  if (!this.monthly_installment && this.repayment_start_date && this.repayment_end_date && this.loan_amount && this.interest_rate) {
    const months = Math.ceil(
      (this.repayment_end_date.getTime() - this.repayment_start_date.getTime()) / 
      (30 * 24 * 60 * 60 * 1000)
    );
    
    if (months > 0) {
      const monthlyRate = this.interest_rate / 100 / 12;
      this.monthly_installment = this.loan_amount * monthlyRate * Math.pow(1 + monthlyRate, months) / 
